...
- Benchmarking Logicle's retrieval/context algorithms (knowledge box vs everything-in-context, and future ones) is done with the goal-driven harness in `apps/backend/lib/eval/**`, run via `apps/backend/scripts/eval.ts`: an LLM impersonates a user pursuing a goal against the real `ChatAssistant`, and arms are compared on success, tokens, cost and break-even. Read [`docs/evaluation-harness.md`](docs/evaluation-harness.md) before changing it — in particular, the simulated user must never be given the scenario's rubric or answer key, or every arm passes.
