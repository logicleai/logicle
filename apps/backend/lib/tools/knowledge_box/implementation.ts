...
export class KnowledgeBoxTool extends KnowledgeBoxInterface implements ToolImplementation {
  ...
  functions_: ToolFunctions = {
    ...
    search: {
      description: `Full-text search across the documents of this knowledge box. For image questions, translate visible landmarks into keywords and search them here; an AI-generated visual index may identify the right file. Returns ranked passages with the document id and the chunk range they came from — use \`read\` with those to see more context, then use \`get_file\` to verify image-dependent or exact claims. At most ${MAX_SEARCH_CALLS_PER_TURN} searches are allowed per user turn. If you do not need to restrict the search, omit \`fileIds\` and search the whole box. Never use ids from conversation attachments or another tool as knowledge-box selectors.`,
      parameters: {
        type: 'object',
        properties: {
          query: {
