...
const usageByUserQuerySchema = z.object({
...
export const GET = operation({
  name: 'Get usage by user',
  description: 'Fetch usage time-series broken down by user for a given period.',
  authentication: 'admin',
  querySchema: usageByUserQuerySchema,
  responses: [
    responseSpec(
      200,
      z.object({
  ...
  implementation: async ({ query }) => {
    ...
    const makeRangeQuery = (from: string, to: string) => {
      let q = db
        ...
        .leftJoin('User', (join) => join.onRef('User.id', '=', 'MessageAudit.userId'))
        .select(sql.lit(from).as('start'))
        .select(sql.lit(to).as('end'))
        .select('MessageAudit.userId as id')
        .select('User.name')
        ...
        .groupBy(['MessageAudit.userId', 'User.name'])
