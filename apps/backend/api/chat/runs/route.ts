...
import { createChatRun, finalizeChatRun, getChatRunById, isChatRunAbortError, persistAndPublishChatRunEvent } from '@/backend/lib/chat/chatRuns'
...
import { error, forbidden, operation, responseSpec, errorSpec, conflict } from '@/lib/routes'
...
export const POST = operation({
  ...
  implementation: async ({ body, headers, session }) => {
    ...
    if (!created.ok) {
      return conflict('A chat run is already active for this conversation', { runId: created.run.id })
    }
    ...
    const saveAndAuditMessage = async (message: dto.Message, usage?: { totalTokens: number; inputTokens: number }) => {
